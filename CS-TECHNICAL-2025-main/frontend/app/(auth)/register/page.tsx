...
export default function RegisterPage() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    // Client-side password policy: at least 8 chars, includes letter and number
    const pwOk = /(?=.*[A-Za-z])(?=.*\d).{8,}/.test(password);
    if (!pwOk) {
      setError("Password too weak — use at least 8 characters, include a letter and a number.");
      setLoading(false);
      return;
    }
    setLoading(true);
      ...
      <div className="max-w-md w-full">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Start tracking your interviews, matches, and career insights.
          </p>
        </div>
